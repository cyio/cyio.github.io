(function(){
    ...
    gulp.task('demo-app', function (cb) {
        ...
        gulp.src(paths.source.root + 'my-app/my-app.less')
            .pipe(less({
              paths: [ path.join(__dirname, 'less', 'includes') ]
            }))
            .pipe(gulp.dest(paths.build.styles));
    ...
    gulp.task('browser-sync', function() {
        browserSync({
            files: ["build/*.html", "build/js/*.js", "build/css/*.css"],
            server: {
                //baseDir: "./examples/template7-pages" // Change this to your web root dir
                baseDir: "./"
            },
            open: false,
            host: "192.168.1.143"
    ...
    gulp.task('open', function () {
        return gulp.src('./index.html').pipe(open({ uri: 'http://localhost:3000/build'}));
    });
