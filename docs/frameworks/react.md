...
- Virtual DOM 树比较是同步、递归进行的，无法中断
- 大量更新时会卡顿

### React 16 的改进

- 任务调度、渲染任务拆分成小的工作单元（fiber node），可以恢复、暂停，避免长时间阻塞主线程

### Virtual DOM vs Fiber

Virtual DOM 是 React 用来描述 UI 的概念结构，而 Fiber 是 React 实现更新、调度和渲染流程的内部机制，是对 Virtual DOM 的更细粒度、可中断、可恢复的实现方式。

> 这种模式和操作系统的「任务调度器」非常像：把整个渲染拆分为一系列"可暂停 + 可恢复 + 有优先级"的任务块（FiberNode）。

> **Virtual DOM 节点只包含 UI 的静态结构信息（type、props 等），而 Fiber Node 不仅包含这些信息，还增加了"调度 + 状态 + 副作用"的执行信息，变成了"可执行的工作单元"。**

### Vue vs React Fiber

Vue 的响应式系统也实现了「更新任务调度 + 精细更新」，相当于部分 Fiber 能力

**Vue 的优势：**
- 简单高效，足够快
- 更小体积，更好维护性
- 实际性能已能满足大多数 SPA 场景
- 真正复杂的并发需求可通过 Web Worker + 主动拆分更新实现

**参考链接：** https://chatgpt.com/share/6875c29c-16b0-8008-b05d-81c4fc5c564c

### 是否过度设计？

React Fiber 的设计目标更偏向于构建**大型、高交互性、动画丰富、响应敏感、异步加载复杂**的 Web 应用，例如：

- 多区域频繁局部更新
- 动画与交互并存的复杂视图
- 渐进式流加载（如 Suspense, RSC）
- 并发用户输入响应

### 举个例子：为什么复杂项目需要 Fiber

假设你做的是一个 AI 写作平台，页面包含：
...
- 编辑器更新触发后，只调度这块区域重新渲染
- 推荐内容加载放低优先级，用 `Suspense` 懒加载
- 用户键盘输入仍优先响应，不会因为低优动画卡顿
- 全局状态变动时，局部 diff，不必整体重渲染

这类复杂依赖频繁、异步密集的场景，是 Fiber 发挥优势的典型战场。
