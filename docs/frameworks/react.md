...
背景：现代应用变得复杂，而 JS 存在单线程阻塞，需要一种任务调度方案

React Fiber 是 React 中重新实现的调和算法，用于提高性能和优化用户界面更新。它通过将工作分解为较小的单元，并允许在不阻塞主线程的情况下暂停和恢复这些工作，从而实现了更平滑的用户体验。

React Fiber 的主要优势包括：
- **优先级调度**：不同类型的更新具有不同的优先级，确保关键任务（如动画）的优先级更高。
- **任务中断和恢复**：能够暂停当前的工作并在之后恢复，从而避免长时间阻塞主线程。
- **任务中止**：如果某些工作不再需要，可以中止这些工作，以节省计算资源。
