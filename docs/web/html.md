...
- 横向浮动布局
  ...
  - 设置了浮动的元素，仍旧处于标准文档流中，意味着会占据文档流的空间，对周围元素（紧邻后面的元素）会产生影响（混排在一起）.浮动元素父级坍塌，没有高度，影响后续元素定位。
  - 莫名奇妙的现象：如果块级元素没有设置宽度，设置浮动后不会显示

- 清除浮动的常用方法

  希望浮动元素不影响非浮动元素，消除对非浮动元素的影响

  - 对浮动元素紧邻后面的元素设置 clear 属性: clear:both;
  - 同时设置：width:100%(或固定);overflow:hidden; 【推荐】触发 BFC
